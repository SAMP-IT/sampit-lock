    ...
    <View style={styles.switcherContainer}>
      ...
      {isOpen && (
        <View style={styles.switcherDropdown}>
          {locks
            .filter((lock) => lock.id !== currentLock.id)
            .map((lock) => (
              <TouchableOpacity
                key={lock.id}
...
const LockDetailScreen = ({ navigation, route }) => {
  ...
  const [isSyncingFromLock, setIsSyncingFromLock] = useState(false);
  ...
  const transformLockData = (lock, permissions = {}) => {
    ...
    const needsRepairing =
      lock.ttlock_mac && !lock.ttlock_data && !lock.is_bluetooth_paired;
    ...
    return {
      ...
      needsRepairing,
      lastActivity: lock.updated_at
        ? new Date(lock.updated_at).toLocaleString()
        : "Unknown",
      ...
      try {
        ...
        const lockWithSettings = {
          ...
          passage_mode_enabled:
            settings.passage_mode_enabled ?? lock.passage_mode_enabled,
        };

        const permissions = lockDetailsData?.permissions || {};
        const transformedLock = transformLockData(
          lockWithSettings,
          permissions,
        );
  ...
  const loadBackgroundData = useCallback(
    async (lock) => {
        ...
        getBatteryFromTTLock(lock.id, lock.ttlock_lock_id)
          .then((batteryFromCloud) => {
            if (batteryFromCloud !== null) {
              setCurrentLock((prev) =>
                prev ? { ...prev, battery: batteryFromCloud } : prev,
              );
            }
          })
      ...
      Promise.all([
        ...
        // Fetch AI insights
        getAIInsights(lockId, { type: "anomaly", limit: 50 }).catch(() => ({
          data: [],
        })),
        ...
        .then(([activitiesResponse, insightsResponse, recsResponse]) => {
          ...
          let recentActivities = (
            Array.isArray(rawActivities) ? rawActivities : []
          ).map((activity) => ({
            ...activity,
          ...
          // Merge AI insights
          const insightsData =
            insightsResponse?.data?.data ?? insightsResponse?.data;
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      if (forceRefresh) {
        console.log(
          "[LockDetailScreen] Force refresh requested, clearing cache",
        );
      ...
      if (lastLoadedLockId.current === lockId && !isInitialLoad.current) {
        console.log(
          "[LockDetailScreen] Same lock, refetching for fresh settings",
        );
        fetchData(true);
        return;
      }
  ...
  // Sync activity from lock via Bluetooth (refresh button in Activity Log)
  const handleRefreshFromLock = useCallback(async () => {
    if (!currentLock || isSyncingFromLock) return;
    const methods = LockControlService.getAvailableControlMethods(currentLock);
    if (!methods.bluetooth && !methods.cloud) return;

    if (syncAbortRef.current) {
      syncAbortRef.current.abort();
    }
    syncAbortRef.current = new AbortController();
    const signal = syncAbortRef.current.signal;

    setIsSyncingFromLock(true);
    try {
      const result = await LockControlService.syncOperationRecords(
        currentLock,
        8000,
        signal,
      );
      ...
      if (result.count > 0) {
        showSuccess(
          `Synced ${result.count} record${result.count === 1 ? "" : "s"} from lock`,
        );
      }
      loadBackgroundData(currentLock);
    } catch (err) {
      showError(err?.message || "Fetch failed. Wake the lock and try again.");
      console.warn("[LockDetailScreen] Sync from lock failed:", err?.message);
    } finally {
      setIsSyncingFromLock(false);
  ...
  const handleUnlock = async () => {
    ...
    if (currentLock.time_restricted || currentLock.userRole === "scheduled") {
      ...
      if (
        currentLock.time_restriction_start &&
        currentLock.time_restriction_end
      ) {
        ...
        if (currentTime < startTime || currentTime > endTime) {
          Alert.alert(
            "Access Restricted",
            `You can only unlock between ${startTime} and ${endTime}.`,
          );
          return;
        }
      }
    ...
    if (
      currentLock.access_valid_until &&
      new Date(currentLock.access_valid_until) < new Date()
    ) {
    ...
    try {
      setIsUnlocking(true);
      console.log(
        `[LockDetailScreen] Unlocking: ${currentLock.name}, role: ${currentLock.userRole}`,
      );
      ...
      if (result.success) {
        console.log(
          `[LockDetailScreen] Unlock successful via ${result.method}`,
        );
  ...
  const handleLock = async () => {
    ...
    try {
      setIsLocking(true);
      console.log(
        `[LockDetailScreen] Locking: ${currentLock.name}, role: ${currentLock.userRole}`,
      );
  ...
  const handleUserManagement = () => {
    ...
    navigation.navigate("UserManagementLock", {
  ...
  return (
    ...
    >
            ...
            <View style={styles.lockHeader}>
              <View style={styles.lockTitleContainer}>
                <Text style={styles.lockName}>
                  {getLockDisplayName(currentLock, "My Lock")}
                </Text>
                <Text style={styles.lockLocation}>{currentLock.location}</Text>
              </View>
              {/* Passage mode indicator - only when enabled */}
              {(currentLock.passage_mode_enabled ||
                currentLock.passageModeEnabled) && (
              ...
              {(currentLock.userRole === "owner" ||
                currentLock.userRole === "admin" ||
                currentLock.can_modify_settings) && (
                <TouchableOpacity
            ...
            {currentLock.needsRepairing && (
              <View style={styles.repairBanner}>
                ...
                <View style={styles.repairBannerContent}>
                  <Text style={styles.repairBannerTitle}>
                    Re-pairing Required
                  </Text>
      ...
      {!["guest", "guest_otp"].includes(currentLock.userRole) && (
        <Section title="Quick Actions" subtitle="Manage access and settings">
            ...
            <FlatList
              data={[
                    ...
                    ? [
                        {
                          ...
                          text: "Passcode",
                          onPress: () =>
                            navigation.navigate("AccessCodeManagement", {
                              lockId: currentLock.id,
                              lockName:
                                currentLock.name || currentLock.location,
                              readOnly: true,
                            }),
                        },
                      ]
                ...
                // Fingerprints - available to all non-guest roles (own fingerprints only for family/resident/scheduled/guest_longterm)
                {
                  ...
                  text: "Fingerprints",
                  onPress: () =>
                    navigation.navigate("FingerprintManagement", {
                      lockId: currentLock.id,
                      lock: currentLock,
                    }),
                },
                // IC Cards - available to owner, admin, family, resident
                ...(!["scheduled", "guest_longterm"].includes(
                  currentLock.userRole,
                )
                  ? [
                      {
                        id: 4,
      ...
      {currentLock.userRole !== "guest" &&
        currentLock.userRole !== "guest_otp" && (
          <Section
            title="Activity Log"
            subtitle="Wake the lock, then tap refresh to get recent data"
            rightAccessory={
              LockControlService.getAvailableControlMethods(currentLock)
                ?.bluetooth ||
              LockControlService.getAvailableControlMethods(currentLock)
                ?.cloud ? (
                <TouchableOpacity
                  onPress={handleRefreshFromLock}
                  disabled={isSyncingFromLock}
                ...
                <>
                  ...
                  {sortedActivity.length > visibleActivitiesCount && (
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        setVisibleActivitiesCount((prev) => prev + 7)
                      }
                    >
